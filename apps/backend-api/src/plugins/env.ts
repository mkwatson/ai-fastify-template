...
const EnvSchema = z.object({
  ...
  }).min(1, 'HOST cannot be empty').default('localhost'),
...
export default fp(async (fastify) => {
  try {
    ...
    fastify.log.info({ config: safeConfig }, 'Environment configuration loaded');
  } catch (error) {
    if (error instanceof z.ZodError) {
      ...
      throw new Error(`Environment validation failed: ${formattedErrors.map(e => `${e.field}: ${e.message}`).join(', ')}`);
    }
