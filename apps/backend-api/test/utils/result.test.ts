  ...
  describe('FastifyResultUtils', () => {
    ...
    describe('toHttpError', () => {
      it('should convert ValidationError to badRequest', () => {
        ...
        expect(mockFastify.httpErrors.badRequest).toHaveBeenCalledWith(
          'Invalid input'
        );
      });
      ...
      it('should convert NotFoundError to notFound', () => {
        ...
        expect(mockFastify.httpErrors.notFound).toHaveBeenCalledWith(
      ...
      it('should convert ConflictError to conflict', () => {
        ...
        expect(mockFastify.httpErrors.conflict).toHaveBeenCalledWith(
          'User already exists'
        );
      });
    ...
    describe('handleResult', () => {
      ...
      it('should throw HTTP error for error result', () => {
        ...
        expect(() =>
          FastifyResultUtils.handleResult(mockFastify, result)
    ...
    describe('wrapHandler', () => {
      it('should wrap service function for automatic error handling', async () => {
        ...
        const wrappedHandler = FastifyResultUtils.wrapHandler(
          mockFastify,
          serviceFunction
        );
      ...
      it('should handle service errors automatically', async () => {
        ...
        await expect(wrappedHandler('input')).rejects.toThrow(
          '400: Invalid input'
        );
      });
    });
  ...
  describe('AsyncResultUtils', () => {
    describe('map', () => {
      it('should map successful async result', async () => {
        ...
        const mapped = await AsyncResultUtils.map(
          asyncResult,
          value => value * 2
        );
    ...
    describe('chain', () => {
      it('should chain async operations', async () => {
        const asyncResult = Promise.resolve(ok(5));
        const chained = await AsyncResultUtils.chain(asyncResult, async value =>
          ok(value * 2)
        );
      ...
      it('should propagate second error', async () => {
        ...
        const asyncResult = Promise.resolve(ok(5));
        const chained = await AsyncResultUtils.chain(asyncResult, async () =>
          err(error)
        );
