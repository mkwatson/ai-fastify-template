...
async function timeCommand(command, description) {
  ...
  try {
    ...
    console.log(`${colorize('green', '✅')} Completed in ${formatTime(duration)}`);
    ...
    const duration = Date.now() - startTime;
    console.log(`${colorize('red', '❌')} Failed after ${formatTime(duration)}`);
...
async function runBenchmarks() {
  ...
  console.log('Measures build performance with no caching (simulates CI environment)\n');
  
  await clearCaches();
  results.coldBuild = await timeCommand('pnpm install && pnpm build', 'Cold build (install + build)');
  ...
  results.incrementalBuild = await timeCommand('pnpm build', 'Incremental build (cached)');
  
  console.log();
  
  // Benchmark 3: AI Quick Feedback Loop (most common during development)
  console.log(colorize('bold', '3️⃣ AI Quick Feedback Loop'));
  console.log('Measures the primary AI development workflow (lint + type-check)\n');
  
  await clearCaches();
  results.aiFeedbackCold = await timeCommand('pnpm ai:quick', 'AI quick feedback (cold)');
  results.aiFeedbackCached = await timeCommand('pnpm ai:quick', 'AI quick feedback (cached)');
  ...
  await fs.writeFile(testFile, '// Benchmark test change\nexport const test = true;\n');
  
  results.affectedLint = await timeCommand('pnpm affected:lint --base=HEAD~1', 'Affected lint (selective)');
  results.affectedAll = await timeCommand('pnpm affected:all --base=HEAD~1', 'Affected all tasks (selective)');
  ...
  results.configValidation = await timeCommand('pnpm test:config:verify', 'Config validation (full)');
  results.configQuick = await timeCommand('node scripts/validate-vitest-configs.js', 'Config validation (quick)');
  ...
  benchmarks.forEach(benchmark => {
    ...
    const status = targetMet ? colorize('green', '✅') : colorize('yellow', '⚠️');
    const comparison = targetMet ? 'MEETS TARGET' : 'EXCEEDS TARGET';
    
    console.log(`${status} ${benchmark.name}: ${formatTime(duration)} (${comparison})`);
  ...
  if (results.aiFeedbackCached.success && results.aiFeedbackCold.success) {
    const cacheSpeedup = results.aiFeedbackCold.duration / results.aiFeedbackCached.duration;
    console.log(`${colorize('cyan', '🚀')} Cache speedup factor: ${cacheSpeedup.toFixed(1)}x`);
  }
  
  if (results.testsCached.success && results.testsCold.success) {
    const testSpeedup = results.testsCold.duration / results.testsCached.duration;
    console.log(`${colorize('cyan', '🧪')} Test cache speedup: ${testSpeedup.toFixed(1)}x`);
  }
  ...
  if (allTargetsMet) {
    ...
    console.log(colorize('green', 'The Nx migration delivered the promised performance improvements!'));
  } else {
    console.log(colorize('yellow', '⚠️ SOME TARGETS NOT MET'));
    console.log(colorize('yellow', 'Performance is good but may not match all claimed improvements.'));
    console.log(colorize('yellow', 'Consider further optimization or adjusting expectations.'));
  }
}
